...
export const calculateNpcStats = (npcSetupData: InitialNpc, cultivationSystem: CultivationTier[], difficulty: GameDifficulty): NpcCharacter => {
    ...
    let baseNpc: NpcCharacter = {
        ...
        nguHanh: npcSetupData.nguHanh ? Object.entries(npcSetupData.nguHanh).filter(([, val]) => val > 0).map(([key, val]) => `${key.charAt(0).toUpperCase() + key.slice(1)} (${val * 20}%)`).join(', ') : 'Không có',
    ...
    newNpc.cultivationQualityId = null;
    newNpc.cultivationStage = `${major.name} ${minor.name}`.trim();
    newNpc.cultivationQuality = null;
    newNpc.level = 1;
    newNpc.exp = 0;
